...
-- controlled-account writing test. v9's block label was not visible inside the embedded SQL expression, so use a distinct local variable.
...
begin
  ...
  if v_cur.is_blank and v_tgt.is_blank then
    ...
    v_review_id_check := review_id;
    if not exists(
      select 1
      from public.followup_review_assets r
      where r.review_id = v_review_id_check
    ) then
      review_id := '833';
    end if;
