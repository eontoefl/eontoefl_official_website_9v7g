    ...
    students.forEach(s => {
        ...
        recentFraud.forEach(fr => {
            ...
            alerts.push({
                ...
                subtitle: `${s.isAustralia ? 'AU ' : ''}${s.programType} ${s.totalWeeks}주 | ${s.currentWeek}주차 | fraud_flag = true`,
                userId: s.userId
            });
        });
        ...
        if (s.consecutiveMissing >= 2) {
            ...
            if (missedDays.length >= 2) {
                alerts.push({
                    ...
                    subtitle: `${s.isAustralia ? 'AU ' : ''}${s.programType} ${s.totalWeeks}주 | ${s.currentWeek}주차 | 인증률 ${s.avgAuthRate}% → 이탈 위험`,
                    userId: s.userId
                });
            }
        }
