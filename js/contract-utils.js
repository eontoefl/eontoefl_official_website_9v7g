...
function parseContractTemplate(template, studentData = {}) {
    ...
    html = html.replace(/\{\{copy:([^}]+)\}\}/g, (match, answer) => {
        ...
        return `<span class="copywrite-container" style="position: relative; display: inline-block; background: linear-gradient(to right, #fef3c7 0%, #fefce8 100%); border-radius: 4px; padding: 0;"><span class="copywrite-hint" id="${displayId}" style="position: absolute; left: 8px; top: 50%; transform: translateY(-50%); color: #d1d5db; pointer-events: none; z-index: 1; font-family: inherit; font-size: inherit; white-space: pre;">${escapeHtml(answer)}</span><input type="text" class="contract-input contract-input-copy" data-answer="${escapeHtml(answer)}" data-field-id="${fieldId}" data-display-id="${displayId}" placeholder="" required oninput="validateCopywrite(this)" style="display: inline-block; width: ${minWidth}px; max-width: 100%; padding: 2px 8px; border: none; border-bottom: 2px solid #eab308; background: transparent; font-family: inherit; font-size: inherit; position: relative; z-index: 2; letter-spacing: 0.02em; color: #1e293b;"><span class="copywrite-status" style="margin-left: 8px; display: inline-block; vertical-align: middle;"></span></span>`;
    });
