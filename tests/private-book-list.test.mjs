...
function setup({allowed=true,rows=[{id,title:'Private title <script>',total_pages:292}],deferred}={}) {
 class Element {constructor(tag){this.tag=tag;this.children=[];this.hidden=false;this.textContent='';}append(...children){this.children.push(...children);}replaceChildren(...children){this.children=children;}}
 const nodes=Object.fromEntries(['privateBookList','privateBookListStatus','privateBookListLogin'].map(k=>[k,new Element('div')]));
 const calls=[],events={};
...
test('authorized card uses private editor/preview and text-only title',async()=>{const x=setup();assert.equal(await x.api.load(),true);assert.deepEqual(x.calls,['ready','pb_book_documents']);const card=x.nodes.privateBookList.children[0];assert.equal(card.children[0].children[0].children[0].textContent,'Private title <script>');const actions=card.children[1].children;assert.equal(actions.length,2);assert.ok(actions.every(a=>a.tag==='a'&&a.href.endsWith('?private=1&book='+id)));assert.equal(x.nodes.privateBookListLogin.hidden,true);});
