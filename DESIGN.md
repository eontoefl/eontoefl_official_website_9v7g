...
*   **학생에게 실제 메일을 보내는 기능(Gmail 발송)은 대표 명시 승인 전까지 금지.** 후보 스캔·초안 생성도 실발송과 묶여 승인 전엔 자동 가동하지 않는다. 이 저장소의 화면·함수는 **조회와 대표의 수정 저장·한 통 취소·지금 발송 요청까지만** 하며, **Gmail 실발송과 전체 발송 잠금(`followup_runtime.send_locked`) 해제는 하지 않는다.** `지금 발송`도 장부에 요청 시각만 남긴다.
...
*   정상 대상은 원래 시점에 초안을 만든 뒤 `awaiting_review`로 발송 예정 목록에 올린다. 대표는 1시간 동안 수정·한 통 취소·지금 발송을 할 수 있고, 아무 작업도 하지 않으면 자동 발송한다.
*   새 초안은 기존 `telegram-notify` 기능으로 한 시간 단위로 묶어 알린다. 알림에는 초안 수와 가장 빠른 발송 예정 시각을 넣는다.
*   정상 운영 중 오래된 새 대상이 발견되면 중대한 파이프라인 사고로 보고, 늦은 메일을 만들지 않은 채 텔레그램으로 즉시 알린다.
